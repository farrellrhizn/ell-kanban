...
import type { ColumnPayload, ColumnRecord } from '../types/kanban.js';
...
router.get('/', async (req: Request, res: Response) => {
  ...
  try {
    const result = await query<ColumnRecord>(
      `SELECT
         id,
         board_id AS "boardId",
         title,
...
router.post('/', async (req: Request, res: Response) => {
  const { title, position, boardId = 1, wipLimit, color } = req.body as ColumnPayload & { title?: string };
  ...
  try {
    const result = await query<ColumnRecord>(
      `INSERT INTO kanban_columns (board_id, title, position, wip_limit, color)
       VALUES (
         $1,
         $2,
         COALESCE(
       ...
       RETURNING id,
                 ...
                 wip_limit AS "wipLimit",
                 color`,
      [boardId, title.trim(), position ?? null, wipLimit ?? null, color ?? null]
    );
...
router.patch('/:id', async (req: Request, res: Response) => {
  ...
  const { title, position, boardId, wipLimit, color } = req.body as ColumnPayload & { title?: string };
    ...
    const result = await query<ColumnRecord>(
      `UPDATE kanban_columns
       ...
       RETURNING id,
                 ...
                 wip_limit AS "wipLimit",
                 color`,
      [title?.trim() ?? null, position ?? null, boardId ?? null, wipLimit ?? null, color ?? null, numericId]
    );
